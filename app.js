document.addEventListener('DOMContentLoaded', () => {
    ...
    processBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            // Số lớp mỗi gói. Hạ xuống 4 lớp/gói thay vì 10 để giảm kích thước text và số lượng yêu cầu mỗi lần.
            // Điều này giúp tỷ lệ bị từ chối 429 giảm hẳn vì dung lượng token/phút trả về nằm trong mức an toàn của Google Gemini.
            let batchClassSize = 4;
...
CHỈ TRẢ VỀ NHỮNG DÒNG CHỨA DẤU |, MỖI TIẾT MỘT DÒNG CHO ĐỦ ${batchTaskCount} MÃ ID, TUYỆT ĐỐI KHÔNG DÙNG FORMAT MARKDOWN HOẶC GIẢI THÍCH GÌ THÊM!`;
                ...
                while (retries > 0) {
                    try {
                        ...
                        if (e.message.includes('429') || e.message.includes('Timeout') || e.message.includes('thử lại') || e.message.includes('fetch')) {
                            statusArea.value = `[Quá tải Server Google] Đang chờ 15s và thử lại vòng lặp (${4 - retries}/3)...`;
                            // Khi gặp lỗi 429, cần phải đợi một khoảng thời gian dài hơn (15 giây) để quota API phục hồi hoàn toàn.
                            await new Promise(r => setTimeout(r, 15000));
                            retries--;
                            if (retries === 0) {
                                for (let t of originalBatchTasks) geminiResults.push({ id: t.id, updatedText: t.text + ` (Lỗi AI: Quá Tải/Chờ Lâu)` });
                            }
                        } else {
                ...
                // Chờ đợi 8 giây chậm rãi sau mỗi vòng lặp THÀNH CÔNG để nhường đường cho mạng 
                // Tránh tình trạng Google Gemini khoá họng (Lock API) vì spam quá nhiều request liên tiếp trong 1 phút.
                if (b < totalBatches - 1) {
